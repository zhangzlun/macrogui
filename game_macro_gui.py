...
全域熱鍵：
    F8  開始 / 暫停技能輪替
    F9  結束程式
    各組合的熱鍵可在組合彈窗中自訂（單鍵或組合鍵，如 F1、Ctrl+Shift+K）
...
class App:
    def __init__(self, root):
        ...
        self.capture_cb = None       # 擷取結果回呼（由組合彈窗提供）
    ...
    def build_ui(self):
        ...
        self.root.grid_rowconfigure(1, weight=3)   # 輪替卡
        self.root.grid_rowconfigure(2, weight=2)   # 組合卡
        ...
        ctk.CTkLabel(head, text="　F8 開始／暫停　各鍵獨立倒數，冷卻到了就按",
        ...
        make_btn(btns, "新增動作", self.add_action, kind="primary",
                 width=92, font=self.f_body).pack(pady=(0, 3))
        make_btn(btns, "編輯", self.edit_action, width=92, font=self.f_body).pack(pady=3)
        make_btn(btns, "刪除", self.del_action, kind="danger", width=92,
                 font=self.f_body).pack(pady=3)
        make_btn(btns, "上移", lambda: self.move_action(-1), width=92,
                 font=self.f_body).pack(pady=3)
        make_btn(btns, "下移", lambda: self.move_action(1), width=92,
                 font=self.f_body).pack(pady=3)
        ...
        make_btn(cbtns, "新增組合", self.add_combo, kind="primary",
                 width=92, font=self.f_body).pack(pady=(0, 3))
        make_btn(cbtns, "編輯", self.edit_combo, width=92, font=self.f_body).pack(pady=3)
        make_btn(cbtns, "刪除", self.del_combo, kind="danger", width=92,
                 font=self.f_body).pack(pady=3)
        ...
        self._entry(orow, self.var_focus, width=170).pack(side="left", padx=6)
        ctk.CTkLabel(orow, text="時才按鍵（留空 = 不限制）", font=self.f_body,
                     text_color=COL_TEXT).pack(side="left")
        ...
        ctk.CTkCheckBox(opt, text="使用 DirectInput（Windows 遊戲建議勾選）",
                        ...
                        corner_radius=5).pack(anchor="w", padx=14, pady=(4, 12))

        # ── 控制列 ──
        ctl = ctk.CTkFrame(self.root, fg_color="transparent")
        ctl.grid(row=4, column=0, sticky="ew", padx=12, pady=4)
        self.btn_start = make_btn(ctl, "▶ 開始輪替（F8）", self.toggle, kind="primary",
        ...
        self.lbl_status = ctk.CTkLabel(
            ctl, text="待命中。F8 開始/暫停輪替，F9 結束（全域有效）",
    ...
    def capture_hotkey_for(self, label, callback):
    ...
    def apply_captured(self, spec):
        ...
        self.end_capture()
        if spec in ("<f8>", "<f9>"):
            self.log("F8/F9 保留給 暫停/結束，請換一個熱鍵")
            return
                ...
                if mode == "cooldown":
                    for i, a in enumerate(m.get("actions", [])):
                        if now >= timers.get(i, now):
                            ...
                            break
                else:  # combo：整組照順序按，鍵與鍵之間隨機延遲，共 N 輪
...
class ComboDialog(BaseModal):
    ...
    def __init__(self, app, combo=None):
        ...
        # 步驟表
        ctk.CTkLabel(self, text="步驟（由上到下依序按，鍵間隨機延遲）",
        ...
        self.tree_s.configure(yscrollcommand=ssb.set, columns=("key", "wmin", "wmax"))
        for cc, h, w in (("key", "按鍵", 120), ("wmin", "最短等待(秒)", 110),
                         ("wmax", "最長等待(秒)", 110)):
            self.tree_s.heading(cc, text=h)
            self.tree_s.column(cc, width=w, anchor="center")
        for s in c.get("steps", []):
            self.tree_s.insert("", "end", values=(
                s.get("key", ""), s.get("wait_min", 0.3), s.get("wait_max", 0.8)))
        self.tree_s.bind("<Double-1>", lambda e: self._edit_step())
        make_btn(sbtns, "新增步驟", self._add_step, kind="primary",
                 width=88, font=f_body).pack(pady=(8, 3))
        make_btn(sbtns, "編輯", self._edit_step, width=88, font=f_body).pack(pady=3)
        make_btn(sbtns, "刪除", self._del_step, kind="danger", width=88,
                 font=f_body).pack(pady=3)
        make_btn(sbtns, "上移", lambda: self._move_step(-1), width=88,
                 font=f_body).pack(pady=3)
        make_btn(sbtns, "下移", lambda: self._move_step(1), width=88,
                 font=f_body).pack(pady=3)
    ...
    # ----- 步驟編輯（AskDialog 疊在本彈窗上，關閉後把 grab 拿回來） -----
    ...
    def _add_step(self):
        vals = self._ask("新增步驟",
                         [("按鍵（如 1、q、space、右鍵）", ""),
                          ("最短等待秒", "0.3"), ("最長等待秒", "0.8")])
        if vals and vals[0].strip():
            self.tree_s.insert("", "end", values=(vals[0].strip(),
                                                  vals[1] or "0.3", vals[2] or "0.8"))

    def _edit_step(self):
        sel = self.tree_s.selection()
        if not sel:
            return
        old = list(self.tree_s.item(sel[0], "values"))
        vals = self._ask("編輯步驟",
                         list(zip(["按鍵", "最短等待秒", "最長等待秒"], old)))
        if vals and vals[0].strip():
            self.tree_s.item(sel[0], values=[vals[0].strip()] + vals[1:])

    def _del_step(self):
        for i in self.tree_s.selection():
    ...
    def _move_step(self, d):
        ...
        if not sel:
            return
        i = self.tree_s.index(sel[0])
        self.tree_s.move(sel[0], "", max(0, i + d))
    ...
    def _save(self):
        ...
        try:
            ...
            steps = [{"key": r[0], "wait_min": float(r[1]), "wait_max": float(r[2])}
                     for r in (self.tree_s.item(i, "values")
